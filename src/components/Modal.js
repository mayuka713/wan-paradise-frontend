        ...
        <form onSubmit={handleSubmit}>
          <div className="rating-container">
            <div className="star-rating">
              {[1, 2, 3, 4, 5].map((value) => (<span key={value} className={`star ${value <= selectedRating ? "selected" : ""}`} onClick={() => setSelectedRating(value)}>
                  ★
                </span>))}
            </div>
          </div>
          <div className="comment-container">
            <textarea className="comment-textarea" value={comment} onChange={(e) => setComment(e.target.value)} placeholder="口コミを入力してください"/>
          </div>
