...
const DogRunPage = () => {
    ...
    useEffect(() => {
        if (!sliderRef.current || stores.length === 0)
            return;
      ...
      <div className="dogrun-page-container">
        ...
        <div className="store-slider-container">
          <div className="dogrun-slider" ref={sliderRef}>
            {stores.map((store, index) => (<div key={`${store.store_id}-${index}`} className="store-card" onClick={() => navigate(`/dogrun/detail/${store.store_id}`)}>
