...
const PetShopStoreList = () => {
    ...
    return (<>
      ...
      <div className="content">
        {selectedPrefecture === "ペットショップ情報がありません" ? (<h2>{selectedPrefecture}</h2>) : (<>
            <h2 className="region-list-title-petshop">{selectedPrefecture}のペットショップ</h2>
            <p className="search-tags">行きたいペットショップの条件を絞り込む</p>
            {/* タグボタンを表示 */}
            <div className="petshop-tags">
              {type4Tag.map((tag) => (<button key={tag.id} onClick={() => handleTagClick(tag.id)} className={`petshop-tag-button ${selectedTagIds.includes(tag.id) ? "selected" : ""}`}>
                  {tag.name}
                </button>))}
            </div>
            {/* エラーメッセージ */}
            {error ? (<p className="error-message">{error}</p>) : (<div className="store-list">
                {/* 店舗がない場合 */}
                {store.length === 0 ? (<p className="no-store-message">
                    該当する店舗が見つかりませんでした。
                  </p>) : (
                /* 店舗がある場合 */
                store.map((storeItem) => {
                    ...
                    const averageRating = reviews.length > 0
                        ...
                        : 0;
                    return (<Link to={`/petshop/detail/${storeItem.store_id}`} className="store-item" key={storeItem.store_id}>
