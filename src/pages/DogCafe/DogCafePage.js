...
const DogCafePage = () => {
    ...
    // アニメーションの設定
    useEffect(() => {
        if (!slideRef.current || stores.length === 0)
            return;
      ...
      <div className="dogcafe-page-container">
        ...
        <div className="store-slider-container">
          <div className="dogrun-slider" ref={slideRef}>
            {stores.map((store, index) => (<div key={`${store.store_id}-${index}`} className="store-card" onClick={() => navigate(`/dogcafe/detail/${store.store_id}`)}>
                {store.store_img.length > 0 && (<img src={store.store_img[0]} alt={store.store_name} className="store-image"/>)}
