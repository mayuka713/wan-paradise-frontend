...
const DogCafeStoreList = () => {
    ...
    <div className="content">
      {selectedPrefecture === "ドッグカフェ情報がありません" ? (<h2>{selectedPrefecture}</h2>) : (<>
        ...
        <div className="type3-tags">
          {type3Tag.map((tag) => (<button key={tag.id} onClick={() => handleTagClick(tag.id)} className={`dogcafe-tag-button ${selectedTagIds.includes(tag.id) ? "selected" : ""}`}>
                  {tag.name}
                </button>))}
        </div>
        {error ? (<p className="error-message">{error}</p>) : (<div className="store-list">
            {/* 店舗がない時 */}
            {store.length === 0 ? (<p>該当するドッグカフェが見つかりませんでした</p>) : (
                //店舗がある時
                store.map((storeItem) => {
                    const reviews = storeItem.reviews ?? [];
                    const totalRating = reviews.reduce((sum, review) => sum + (review.rating || 0), 0);
                    const averageRating = reviews.length > 0 ? totalRating / reviews.length : 0;
                    return (<Link to={`/dogcafe/detail/${storeItem.store_id}`} className="store-item" key={storeItem.store_id}>
