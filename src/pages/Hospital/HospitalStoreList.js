...
const HospitalStoreList = () => {
    ...
    const [type5Tag, setType5Tag] = useState([]);
    ...
    return (<>
      ...
      <div className="content">
        {selectedPrefecture === "動物病院の情報がありません" ? (<h2>{selectedPrefecture}</h2>) : (<>
            ...
            <div className="type5-tags">
              {/* type1Tagを表示 */}
              {type5Tag.map((tag) => (<button key={tag.id} onClick={() => handleTagClick(tag.id)} className={`hospital-tag-button ${selectedTagIds.includes(tag.id) ? "selected" : ""}`}>
                  {tag.name}
                </button>))}
            </div>
            ...
            {error ? (<p className="error-message">{error}</p>) : (<div className="store-list">
                {/* 店舗がない場合 */}
                {store.length === 0 ? (<p>該当する病院はありません。</p>) : (store.map((storeItem) => {
                    const reviews = storeItem.reviews ?? [];
                    const totalRating = reviews.reduce((sum, review) => sum + (review.rating || 0), 0);
                    const averageRating = reviews.length > 0 ? totalRating / reviews.length : 0;
                    return (<Link to={`/hospital/detail/${storeItem.store_id}`} className="store-item" key={storeItem.store_id}>
                        {/* 店舗画像 */}
